    ...
    <div className="flex flex-col gap-3">
                ...
                {gaps.map((gap) => (
                  <tr key={gap.controlId} className="border-line border-t align-top">
                    ...
                    <td className="py-2 pr-3">
                      ...
                      <span className="text-faint block text-xs leading-snug">{gap.title}</span>
                    </td>
      ...
      {remediation.length > 0 && (
        ...
        >
            ...
            {remediation.map((option) => (
              ...
              >
                <div className="flex flex-wrap items-baseline gap-2">
                  ...
                  <span className="tabular text-muted ml-auto text-sm">
          ...
          <dl className="border-line mt-3 grid grid-cols-2 gap-x-4 gap-y-1 border-t pt-3 text-sm sm:grid-cols-3">
            <div>
              <dt className="text-faint text-2xs tracking-wide uppercase">Annual procurement</dt>
              <dd className="tabular text-ink">{formatMoney(coverage.remediationAnnualSpend)}</dd>
            </div>
            <div>
              <dt className="text-faint text-2xs tracking-wide uppercase">One-time</dt>
              <dd className="tabular text-ink">{formatMoney(coverage.remediationOneTime)}</dd>
            </div>
            <div>
              <dt className="text-faint text-2xs tracking-wide uppercase">Additional effort</dt>
