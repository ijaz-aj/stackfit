...
function ChangeTable({
  ...
  if (rows.length === 0) {
    return <p className="text-faint text-[12px]">No differences.</p>;
  }
    ...
    return (
      <main className="mx-auto flex w-full max-w-[900px] flex-col gap-3 px-5 py-6">
        <header className="flex items-baseline gap-3">
          ...
          <h1 className="text-ink text-[15px] font-semibold tracking-tight">Compare</h1>
        </header>
          ...
          {readable.length < 2 ? (
            ...
            <form className="flex flex-wrap items-end gap-2" method="get">
              <label className="flex flex-col gap-1">
                <span className="text-faint text-[11px]">Left</span>
                <select
                  name="left"
                  defaultValue={leftId ?? readable[0]?.id}
                  className="border-line bg-panel text-ink rounded border px-2 py-1 text-[12px]"
                >
              ...
              <label className="flex flex-col gap-1">
                <span className="text-faint text-[11px]">Right</span>
                <select
                  name="right"
    ...
    <main className="mx-auto flex w-full max-w-[1100px] flex-col gap-3 px-5 py-4">
      <header className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          <Link href={`/scenarios/${leftRow.id}/results`} className="text-accent text-[11px]">
            {comparison.leftName} results
          </Link>
          <span className="text-faint text-[11px]">·</span>
          <Link href={`/scenarios/${rightRow.id}/results`} className="text-accent text-[11px]">
      ...
      <Card title="What changed" hint="Stated, not interpreted. What it means is the analyst's call.">
        <ul className="flex flex-col gap-1">
          {comparison.summary.map((line) => (
            <li key={line} className="text-muted text-[12px] leading-snug">
              {line}
            </li>
          ))}
        </ul>
            ...
            <tbody>
              {comparison.categories.map((change) => (
                <tr key={change.category} className="border-line border-t">
                  ...
                  <td className="py-1.5">
                    {change.kind === 'same' ? (
                      <span className="text-faint text-[11px]">same</span>
                    ) : (
