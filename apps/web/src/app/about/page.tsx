...
export const metadata: Metadata = {
  title: 'About StackFit',
};

/**
 * What StackFit is, how it works, and what it will not do.
 *
 * **This was `/`, and it was public.** It is now behind `requireAnalyst()`,
 * because a page that explains at length what the company scopes, which
 * vendors it prices, what a stack costs a client to run and where the way in is
 * is written for a colleague and reads as reconnaissance to anybody else. The
 * hostname is public knowledge whatever this page says — every Vercel
 * certificate is published to Certificate Transparency logs within minutes of
 * being issued, and bots scrape those continuously — so the control that
 * matters is not what the page says but who can read it. `/` is now a door with
 * nothing written on it; this is what is behind it.
 *
 * The two rules it carried as a public route still hold, and are still worth
 * stating, because they are about what *belongs* here rather than about who is
 * reading:
 *
 ...
 *    our fee, our cost base, our margin — and this page has no business
 *    knowing any of it. The figures in the specimen are a worked example,
 *    written out, and labelled as one.
 *
 * Both are asserted in `test/public-route.test.ts`.
 *
 ...
 * page for a tool whose second hard rule is "never invent a number" should not
 * contain a number nobody can check, and a figure typed into prose is stale the
 * first time someone adds a product.
 *
 * No `revalidate` any more. It had one while it was public and static; a gated
 * route reads the session on every request and is dynamic by construction, so
 * `today()` is a live clock reading again and the staleness that export existed
 * to prevent cannot occur.
 */

/**
 * Never prerendered, the same as every other gated page here.
 *
 * Not a preference — the build fails without it, and it failed on the commit
 * that moved this page behind the gate. `next build` runs with
 * `NODE_ENV=production` and no secrets, and the first thing `requireAnalyst()`
 * does is `assertAuthConfiguredInProduction()`, which is *designed* to throw in
 * exactly that situation. Prerendering this page therefore executes the guard
 * that exists to stop an unauthenticated production instance, at build time,
 * where there is nothing to protect and nobody to protect it from, and kills
 * the build with "Refusing to start: authentication is not configured".
 *
 * `/scenarios` and `/compare` both carry this line for the same reason. The
 * reason this page needed it *added* is that it spent a day as `/`, public and
 * static, where prerendering was the whole point.
 */
  ...
  return (
    ...
    */
    <div className={`${display.variable} display-serif bg-ground`}>
      {/*
        No masthead of its own any more.

        This page carried one while it was `/` and public, because `SiteHeader`
        deliberately declines to render there: every control it holds is for
        somebody already inside. Behind the gate that reasoning inverts. The
        reader *is* somebody already inside, `SiteHeader` renders above this,
        and a second wordmark under the first one reads as a page that failed
        to load.
      */}
        ...
        <section className="flex flex-col gap-11 py-16 sm:py-20">
          <div className="flex flex-col gap-6">
            ...
            */}
            ...
            {/*
              The caption used to say who the portal is for, because a stranger
              was reading. Behind the gate that is answered: whoever is here got
              past the door. So it says what the button does instead.
            */}
            <div className="mt-1 flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-5">
              ...
              <p className="text-faint max-w-[34ch] text-xs leading-relaxed">
                Straight to the scenarios you have saved.
              </p>
            </div>
          </div>
